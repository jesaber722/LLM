...
def parse_file(filename, args):
	...
	for answer in answers:
		if answer == "ERROR":
			continue
		choice, unparsed_percents = answer.split('###')
		if ord(choice) - 65 >= len(choice_counts) or ord(choice) - 65 < 0:
			continue
		choice_counts[ord(choice) - 65] += 1
		perc_strs = unparsed_percents.split(',')
		vote_percents = [float(p) for p in perc_strs]
		perc_total = sum(vote_percents)
		if perc_total != 0:
			vote_percents = [p/perc_total for p in vote_percents]
		else:
			vote_percents = [1/len(vote_percents) for i in range(len(vote_percents))]
		percents.append(vote_percents)
		total += 1
	#print(str(percents))
	#print(str(choice_counts))
	true_answer = chr(correct_answer)
	plur_winner = plurality(choice_counts)
	sp_winner = surprisingly_popular(choice_counts, percents)
