...
def parse_file(filename, args):
	...
	start = start.split('###')
	question = start[2]
	parsed.append([start[3], '-1', False])
	correct = start[4].split(';')
	for ans in correct:
		parsed.append([ans, '-1', False])
	#print(str(parsed))
	answers = answers[2:]
	#print(answers[0])
	for i in range(len(answers)):
		...
		if ans_ind == -1 or perc_ind == -1:
			...
			percent_stuff = answers[i][perc_ind + 8:].strip()
			match = re.search(r'\d+(\.\d+)?%', percent_stuff)
			if match == None:
				parsed.append([])
			else:
				num = match.group()
				parsed.append([answers[i][ans_ind + 7: perc_ind].strip(), num[0:len(num)-1], True])
	file.close()
	label_answers(question, parsed)
	wr_file = open(DATA_PATH + args.destination + "\\" + filename, 'w', encoding='UTF-8')
	for answer in parsed:
		if len(answer) == 0:
			wr_file.write("ERROR\n")
		else:
			wr_file.write(answer[0] + "###" + answer[1] + "###" + str(answer[2]) + "###" + str(answer[3]) + "\n")
	wr_file.close()
	#print(str(parsed[7]))
