...
def parse_file(filename, args):
	...
	for i in range(len(answers)):
		...
		perc_ind = answers[i].find("PERCENTS:")
		if ans_ind == -1 or perc_ind == -1:
			print("bad format")
			#print(str(i)+ answers[i])
			parsed.append([])
		else:
			if len(answers[i][ans_ind + 7: perc_ind].strip()) != 1:
				answers.append([])
				print("extra answer stuff")
				#print(str(i)+ answers[i])
				continue
			percent_stuff = answers[i][perc_ind + 9:].strip()
			nums = re.findall(r'(\d+(\.\d+)?)%', percent_stuff)
			nums = [num[0] for num in nums]
			if len(nums) != number_of_answers:
				print("wrong number of percents")
				#print(str(i)+answers[i])
				parsed.append([])
			else:
				parsed.append([answers[i][ans_ind + 7: perc_ind].strip(), nums])
	file.close()
