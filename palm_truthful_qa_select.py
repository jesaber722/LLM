...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 14 12:43:46 2023
...
def read(file):
    ...
    df = pd.read_csv(file, names=[x for x in range(numAnswers + 1)])
    ...
    for i in df.index:
        ...
        for j in range(1, len(df.iloc[i])):
            if (df.iloc[i, j] >= 0):
                ...
                p[ansInt][j - 1] += (0 - p[ansInt][j - 1]) / c[ansInt]
...
"""Main loop"""
path = 'palm_truthful_qa_preprocessed/'
...
with open('palm_truthful_qa_results.txt', 'w') as f:
    ...
    f.write('* Number of percents given often don\'t match number of answer choices\n')
    f.write('(Probably want to make prompt clearer or add more examples)')